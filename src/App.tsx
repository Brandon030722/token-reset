...
function EventCard({ event, snapshot }: { event: ResetEvent; snapshot: Snapshot }) {
  return <article className="event-card"><div className="event-meta"><span className={event.type === 'limited-reset' ? 'scope limited' : 'scope'}>{event.announcement ? '重置公告' : event.type === 'limited-reset' ? '限定人群' : event.type === 'credit-grant' ? '重置机会' : '广泛重置'}</span><time dateTime={event.announcedAt}>{formatTime(event.announcedAt)}</time></div>
